...
public class FloatEffectCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager.literal("feffect").requires((serverCommandSource) -> {
            return serverCommandSource.hasPermissionLevel(2);
        })).then(((LiteralArgumentBuilder)CommandManager.literal("clear").executes((commandContext) -> {
            return executeClear(commandContext.getSource(), ImmutableList.of(((ServerCommandSource)commandContext.getSource()).getEntityOrThrow()));
            ...
            return executeClear(commandContext.getSource(), EntityArgumentType.getEntities(commandContext, "targets"), MobEffectArgumentType.getMobEffect(commandContext, "effect"));
        ...
        })).then(((RequiredArgumentBuilder)CommandManager.argument("seconds", IntegerArgumentType.integer()).executes((commandContext) -> {
        ...
        })).then(((RequiredArgumentBuilder)CommandManager.argument("amplifier", DoubleArgumentType.doubleArg()).executes((commandContext) -> {
            return executeGive((ServerCommandSource)commandContext.getSource(), EntityArgumentType.getEntities(commandContext, "targets"), MobEffectArgumentType.getMobEffect(commandContext, "effect"), IntegerArgumentType.getInteger(commandContext, "seconds"), DoubleArgumentType.getDouble(commandContext, "amplifier"), true);
        })).then(CommandManager.argument("hideParticles", BoolArgumentType.bool()).executes((commandContext) -> {
            return executeGive((ServerCommandSource)commandContext.getSource(), EntityArgumentType.getEntities(commandContext, "targets"), MobEffectArgumentType.getMobEffect(commandContext, "effect"), IntegerArgumentType.getInteger(commandContext, "seconds"), DoubleArgumentType.getDouble(commandContext, "amplifier"), !BoolArgumentType.getBool(commandContext, "hideParticles"));
        }))))))));
    }

    private static int executeGive(ServerCommandSource source, Collection<? extends Entity> targets, StatusEffect effect, @Nullable Integer seconds, double amplifier, boolean showParticles) throws CommandSyntaxException {
        int i = 0;
        int m;
        if (seconds != null) {
            if (effect.isInstant()) {
        ...
        while(var8.hasNext()) {
            ...
            if (entity instanceof LivingEntity) {
                StatusEffectInstance statusEffectInstance = new FloatStatusEffectInstance(effect, m, amplifier, false, showParticles, showParticles);
